...
VALUES
   ...
   -- Selecting products with the price higher than 20
   SELECT name
   from product
   WHERE price > 20;
   
   -- Selecting products from the same manufacturer
   SELECT name
   from product
   where manufacturer = 'Mad Inventors Inc.';
   
   -- Selecting all the information from the products by price and manufacturer 
   SELECT *
   from product
   where price < 30 AND
   manufacturer = 'Mad Inventors Inc.';
   
   -- Counting the columns in the table 
   SELECT COUNT(*) from product;
   
   -- Selecting average price from a manufacturer and labeling it "avg_price"
   select AVG(price) AS avg_price
   from product
   where manufacturer = 'Mad Inventors Inc.';
   
   -- counting the products by manufacturer 
